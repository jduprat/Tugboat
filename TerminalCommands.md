...
```bash
defaults write io.github.jduprat.Tugboat alternateDefaultShortcuts -bool true
```
...
```bash
defaults write io.github.jduprat.Tugboat subsequentExecutionMode -int 2
```
...
2: disabled
3: cycle displays for left/right actions, halves to thirds for the rest (old Tugboat behavior)
...
```bash
defaults write io.github.jduprat.Tugboat centerHalfCycles -int 1
```
...
```bash
defaults write io.github.jduprat.Tugboat resizeOnDirectionalMove -bool true
```
...
```bash
defaults write io.github.jduprat.Tugboat halvesPreserveOtherAxisSize -bool true
```
...
```bash
defaults write io.github.jduprat.Tugboat stageSize -float <VALUE>
```

To set it to a proportion of your screen's width, set it to a value between 0 and 1.

```bash
defaults write io.github.jduprat.Tugboat stageSize -float <VALUE_BETWEEN_0_AND_1>
```

## Enable Todo Mode

See the [wiki](https://github.com/rxhanson/Tugboat/wiki/Todo-Mode) for more info.

```bash
defaults write io.github.jduprat.Tugboat todo -int 1
```
...
```bash
defaults write io.github.jduprat.Tugboat snapModifiers -int 1048576
```
...
```bash
defaults write io.github.jduprat.Tugboat almostMaximizeHeight -float <VALUE_BETWEEN_0_&_1>
```

```bash
defaults write io.github.jduprat.Tugboat almostMaximizeWidth -float <VALUE_BETWEEN_0_&_1>
```

## Repeated Maximize restores the previous size and position

By default, executing "Maximize" or "Almost Maximize" on a window that is already in that state does nothing. With this enabled, executing the same action again on a window that Tugboat has just maximized (or almost maximized) moves the window back to the size and position it had right before, so the shortcut toggles between the two. A window that was moved or resized by other means in between is maximized as usual.
...
```bash
defaults write io.github.jduprat.Tugboat repeatedMaximizeRestoresPrevious -bool true
```
...
```bash
defaults write io.github.jduprat.Tugboat specified -dict-add keyCode -float 8 modifierFlags -float 1966080
```

```bash
defaults write io.github.jduprat.Tugboat specifiedHeight -float 1050
defaults write io.github.jduprat.Tugboat specifiedWidth -float 1680
```
...
```bash
defaults write io.github.jduprat.Tugboat centerProminently -dict-add keyCode -float 8 modifierFlags -float 1835305
```
...
```bash
defaults write io.github.jduprat.Tugboat topLeftNinth -dict-add keyCode -float 18 modifierFlags -float 917504
```
...
```bash
defaults write io.github.jduprat.Tugboat topLeftEighth -dict-add keyCode -float 18 modifierFlags -float 917504
```
...
```bash
defaults write io.github.jduprat.Tugboat topLeftThird -dict-add keyCode -float 18 modifierFlags -float 917504
```
...
```bash
defaults write io.github.jduprat.Tugboat doubleWidthRight -dict-add keyCode -float 124 modifierFlags -float 11403555
```
...
```bash
defaults write io.github.jduprat.Tugboat cascadeActiveApp -dict-add keyCode -float 2 modifierFlags -float 393475
```
...
```bash
defaults write io.github.jduprat.Tugboat footprintAlpha -float <VALUE_BETWEEN_0_&_1>
```

Change the border width. Default is 2 (used to be 1).

```bash
defaults write io.github.jduprat.Tugboat footprintBorderWidth -float <NUM_PIXELS>
```

Disable the fade.

```bash
defaults write io.github.jduprat.Tugboat footprintFade -int 2
```

Change the color.

```bash
defaults write io.github.jduprat.Tugboat footprintColor -string "{\"red\":0,\"blue\":0.5,\"green\":0.5}"
```

Change the animation duration. The value is a multiplier. Default is 0 (no animation).

```bash
defaults write io.github.jduprat.Tugboat footprintAnimationDurationMultiplier -float <MULTIPLIER>
```
...
```bash
defaults write io.github.jduprat.Tugboat centeredDirectionalMove -int 2
```
...
Set either value to `0` to disable Tugboat's screen-relative limit for that dimension and rely on the application's native minimum size.

```bash
defaults write io.github.jduprat.Tugboat minimumWindowWidth -float <VALUE_BETWEEN_0_&_1>
```

```bash
defaults write io.github.jduprat.Tugboat minimumWindowHeight -float <VALUE_BETWEEN_0_&_1>
```
...
```bash
defaults write io.github.jduprat.Tugboat sizeOffset -float <NUM_PIXELS>
```
...
```bash
defaults write io.github.jduprat.Tugboat curtainChangeSize -int 2
```
...
```bash
defaults write io.github.jduprat.Tugboat largerWidth -dict-add keyCode -float 30 modifierFlags -float 786432
defaults write io.github.jduprat.Tugboat smallerWidth -dict-add keyCode -float 33 modifierFlags -float 786432
```
...
```bash
defaults write io.github.jduprat.Tugboat largerHeight -dict-add keyCode -float 30 modifierFlags -float 917504
defaults write io.github.jduprat.Tugboat smallerHeight -dict-add keyCode -float 33 modifierFlags -float 917504
```
...
```bash
defaults write io.github.jduprat.Tugboat smallerShrinksMaximizedHeight -bool true
```

## Disabling window restore when moving windows

```bash
defaults write io.github.jduprat.Tugboat unsnapRestore -int 2
```
...
```bash
defaults write io.github.jduprat.Tugboat snapEdgeMarginTop -int 10
defaults write io.github.jduprat.Tugboat snapEdgeMarginBottom -int 10
defaults write io.github.jduprat.Tugboat snapEdgeMarginLeft -int 10
defaults write io.github.jduprat.Tugboat snapEdgeMarginRight -int 10
```
...
```bash
defaults write io.github.jduprat.Tugboat screenEdgeGapTop -int 10
defaults write io.github.jduprat.Tugboat screenEdgeGapBottom -int 10
defaults write io.github.jduprat.Tugboat screenEdgeGapLeft -int 10
defaults write io.github.jduprat.Tugboat screenEdgeGapRight -int 10
```
...
```bash
defaults write io.github.jduprat.Tugboat screenEdgeGapTopNotch -int 5
```
...
```bash
defaults write io.github.jduprat.Tugboat screenEdgeGapsOnMainScreenOnly -bool true
```
...
```bash
defaults write io.github.jduprat.Tugboat ignoredSnapAreas -int 1
```

To disable the Top Half and Bottom Half snap areas, the bit field would be 1111 0000 0000, or 3840

```bash
defaults write io.github.jduprat.Tugboat ignoredSnapAreas -int 3840
```
...
```bash
defaults write io.github.jduprat.Tugboat applyGapsToMaximize -int 2
```

To disable the gaps for "Maximize Height", execute:

```bash
defaults write io.github.jduprat.Tugboat applyGapsToMaximizeHeight -int 2
```
...
```bash
defaults write io.github.jduprat.Tugboat sixthsSnapArea -bool true
```
...
```bash
defaults write io.github.jduprat.Tugboat moveCursor -int 1
```

## Control Enhanced UI handling

Some apps enable the macOS `AXEnhancedUserInterface` accessibility mode. Tugboat disables this mode while moving or resizing a window because its animated window updates can otherwise produce incorrect frames. Chromium browsers can also enable expensive web accessibility processing when the mode is restored.

Tugboat uses automatic handling by default: it restores Enhanced UI for other apps, but leaves it disabled after Tugboat window actions, when known Chromium browser families activate, and when one is already frontmost as Tugboat starts. VoiceOver and Switch Control keep the prior restore behavior. The behavior can be overridden with:

```bash
defaults write io.github.jduprat.Tugboat enhancedUI -int <MODE>
```
...
| Mode | Behavior |
|------|----------|
| `1` | Always restore Enhanced UI after a Tugboat window action if it was enabled beforehand. Use this when assistive software depends on the mode. |
...
```bash
defaults write io.github.jduprat.Tugboat missionControlDragging -int 2
```

Change the allowed off-screen distance. The value is in pixels and is tied to the speed. Default is 25.

```bash
defaults write io.github.jduprat.Tugboat missionControlDraggingAllowedOffscreenDistance -float <DISTANCE>
```

Change the disallowed duration. The value is in milliseconds. Default is 250.

```bash
defaults write io.github.jduprat.Tugboat missionControlDraggingDisallowedDuration -int <DURATION>
```

## Change the behavior of double-click window title bar

To change the action ([list](https://github.com/rxhanson/Tugboat/blob/master/Tugboat/WindowAction.swift)):

```bash
defaults write io.github.jduprat.Tugboat doubleClickTitleBar -int <ACTION_ID + 1>
```

To disable restore when double-clicked again:

```bash
defaults write io.github.jduprat.Tugboat doubleClickTitleBarRestore -int 2
```

To disable double-click window title bar only for specific bundle ids (in example, Outlook):

```bash
defaults write io.github.jduprat.Tugboat doubleClickTitleBarIgnoredApps -string "[\"com.microsoft.Outlook\"]"
```
...
```bash
defaults write io.github.jduprat.Tugboat screensOrderedByX -int 1
```
...
```bash
defaults write io.github.jduprat.Tugboat autoMaximize -int 2
```

To restore the default behavior:

```bash
defaults write io.github.jduprat.Tugboat autoMaximize -int 0
```

## Attempt to preserve window position when moving to another display

By default, moving a window to the next, previous, or a specific display centers it on the destination display. Enable this to instead try preserving the window's position on the destination. If the previous action was a Tugboat snap (half, third, maximize, etc.), that snap is replayed on the destination display. If the window was positioned manually, its rect is mapped proportionally from the source display to the destination display (a window at the right third stays at the right third) and clamped so it never overflows. This is off by default.

```bash
defaults write io.github.jduprat.Tugboat attemptMatchOnNextPrevDisplay -int 1
```

To disable it again:

```bash
defaults write io.github.jduprat.Tugboat attemptMatchOnNextPrevDisplay -int 2
```
...
```bash
defaults write io.github.jduprat.Tugboat cyclingOverlapOffset -bool true
```

The default offset is 11pt. To customize the offset size:

```bash
defaults write io.github.jduprat.Tugboat cyclingOverlapOffsetSize -float 16
```

By default, only one cascade layer is shown (the original window plus one offset). To allow more layers:

```bash
defaults write io.github.jduprat.Tugboat cyclingOverlapMaxCascade -int 3
```
...
```bash
defaults write io.github.jduprat.Tugboat stackBadge -bool true
```
...
```bash
defaults write io.github.jduprat.Tugboat moveFixedSizeToEdge -int 1  # align edges and corners (default)
defaults write io.github.jduprat.Tugboat moveFixedSizeToEdge -int 2  # align corners only, center halves/sides
defaults write io.github.jduprat.Tugboat moveFixedSizeToEdge -int 3  # center within the snap area
```
